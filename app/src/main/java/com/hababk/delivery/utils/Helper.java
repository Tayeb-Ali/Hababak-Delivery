package com.hababk.delivery.utils;
...
public class Helper {
    ...
    public static void preLoadCategories(final SharedPreferenceUtil sharedPreferenceUtil) {
        ApiUtils.getClient().create(ChefService.class).getMenuItemCategories(getApiToken(sharedPreferenceUtil)).enqueue(new Callback<BaseListModel<MenuItemCategory>>() {
            @Override
    ...
    public static DeliveryProfile getDeliveryDetails(SharedPreferenceUtil sharedPreferenceUtil) {
        DeliveryProfile toReturn = new DeliveryProfile(true);
        String savedInPrefs = sharedPreferenceUtil.getStringPreference(Constants.KEY_PROFILE, null);
        if (savedInPrefs != null) {
            toReturn = new Gson().fromJson(savedInPrefs, new TypeToken<DeliveryProfile>() {
            }.getType());
        }
        return toReturn;
    }

    public static void setDeliveryDetails(SharedPreferenceUtil sharedPreferenceUtil, DeliveryProfile body) {
        sharedPreferenceUtil.setStringPreference(Constants.KEY_PROFILE, new Gson().toJson(body, new TypeToken<DeliveryProfile>() {
        }.getType()));
    }
    ...
    public static void setLastLocation(SharedPreferenceUtil sharedPreferenceUtil, LatLng latLng) {
        sharedPreferenceUtil.setStringPreference(Constants.KEY_LOCATION, new Gson().toJson(latLng, new TypeToken<LatLng>() {
        }.getType()));
    }

    public static LatLng getLastLocation(SharedPreferenceUtil sharedPreferenceUtil) {
        LatLng toReturn = null;
        String savedInPrefs = sharedPreferenceUtil.getStringPreference(Constants.KEY_LOCATION, null);
        if (savedInPrefs != null) {
            toReturn = new Gson().fromJson(savedInPrefs, new TypeToken<LatLng>() {
            }.getType());
        }
        return toReturn;
    }
